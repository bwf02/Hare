...
#include <sparse_gemm/common/types.hpp>
...
template <GemmType kGemmType,
          uint32_t BLOCK_M, uint32_t BLOCK_N,
          uint32_t NUM_ROW_BLOCK, uint32_t NUM_COL_BLOCK,
...
struct SSDScheduler {
    ...
    // block position
    int *row_indices, *column_indices, *height_offsets;
    int num_col_integer_block;
    // For SM90 multicast checks
    uint32_t num_blocks_in_group;
    bool is_peer_cta_alive = true;

    __device__ __forceinline__ explicit SSDScheduler(int *row_indices, int *column_indices, int *height_offsets) {
        if constexpr (kGemmType == GemmType::Normal) {
            num_blocks = NUM_ROW_BLOCK * NUM_COL_BLOCK;
        }
        this->row_indices = row_indices;
        this->column_indices = column_indices;
        this->height_offsets = height_offsets;

        this->num_col_integer_block = NUM_COL_BLOCK - NUM_EXPERTS;
    }

    __device__ __forceinline__ void get_swizzled_block_idx(const uint32_t& block_idx, uint32_t& m_index, uint32_t& n_index, uint32_t& num_cols) {
        // 目前根据row indices 和 column indices 目前不需要进行cta swizzle
        int n_block_idx = block_idx / NUM_ROW_BLOCK;
        m_index = __ldg(row_indices + block_idx);
        n_index = __ldg(column_indices + n_block_idx);

        // m_index = 0;
        // if (block_id_m > 0) m_index = __ldg(height_offsets + block_id_m - 1);
        // n_index = block_id_n * BLOCK_N; // BLOCK_N = 128
        num_cols = 128; // BLOCK_M = 128
        if (n_block_idx >= num_col_integer_block)
        {
            num_cols = __ldg(height_offsets + (n_block_idx - num_col_integer_block));
        }
    }
    ...
    __device__ __forceinline__ bool get_next_block(uint32_t& m_index, uint32_t& n_index, uint32_t& num_cols) {
        const auto next_block_idx = (++ current_iter) * gridDim.x + blockIdx.x;

        if (next_block_idx >= num_blocks)
            return false;

        get_swizzled_block_idx(next_block_idx, m_index, n_index, num_cols);

        return true;
    }

    __device__ __forceinline__ bool is_computation_valid() const {
        if constexpr (kGemmType == GemmType::Normal) {
            return true;
        }
    }
...
struct DSScheduler
{
    ...
    __device__ __forceinline__ explicit DSScheduler(int *row_indices, int *column_indices, int *height_offsets) {
        this->num_m_blocks = ceil_div(SHAPE_M, BLOCK_M);
        this->num_n_blocks = NUM_COL_BLOCK;
        this->num_blocks = num_m_blocks * num_n_blocks;
    ...
    __device__ __forceinline__ void get_swizzled_block_idx(const uint32_t& block_idx, uint32_t& m_index, uint32_t& n_index, uint32_t& num_cols, uint32_t& k_index) {
        ...
        // Convert to final M/N block indices
        int m_block_idx = in_group_idx / num_blocks_in_group;
        int n_block_idx = first_block_idx + in_group_idx % num_blocks_in_group;
        // if constexpr (kIsMulticastOnA) {
        //     int m_block_idx = in_group_idx / num_blocks_in_group;
        //     int n_block_idx = first_block_idx + in_group_idx % num_blocks_in_group;
        // } else {
        //     int m_block_idx = first_block_idx + in_group_idx % num_blocks_in_group;
        //     int n_block_idx = in_group_idx / num_blocks_in_group;
        // }

        m_index = m_block_idx * BLOCK_M;
        n_index = __ldg(column_indices + n_block_idx);

        num_cols = 128; // BLOCK_N = 128
        if (n_block_idx >= num_col_integer_block) {
            num_cols = __ldg(height_offsets + (n_block_idx - num_col_integer_block));
        }

        k_index = __ldcg(row_indices + n_block_idx * NUM_ROW_BLOCK);
    }
    ...
    __device__ __forceinline__ bool get_next_block(uint32_t& m_index, uint32_t& n_index, uint32_t& num_cols, uint32_t& k_index) {
        const auto next_block_idx = (++ current_iter) * gridDim.x + blockIdx.x;

        if (next_block_idx >= num_blocks)
            return false;

        get_swizzled_block_idx(next_block_idx, m_index, n_index, num_cols, k_index);
