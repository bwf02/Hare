    ...
    )
{
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_index, n_index, num_cols)) {
            uint32_t col = (warp_idx - kNumWarpMath) * 32 + lane_idx;
            ...
            for (uint32_t k_iter = 0; k_iter < num_iterations; ++k_iter){
                ...
                if (col < num_cols){
                    uint32_t origin_col = __ldg(gather_index + n_index + col);
                    cde::cp_async_bulk_tensor_1d_global_to_shared(smem_b[stages_id] + col * BLOCK_K, &b_desc, origin_col * SHAPE_K + k_idx, full_barrier);
                    load_size += BLOCK_K * sizeof(__nv_bfloat16);
                }
