...
{
    static constexpr int PREFETCHEDCOL = kNumMathThreads;
    ...
    // Configs
    const uint32_t num_iterations = ceil_div(NNZ_K * 2, BLOCK_K);
    ...
    // Block scheduler
    uint32_t m_index, n_index, num_cols;
    auto scheduler = SSDScheduler<kGemmType, BLOCK_M, BLOCK_N,
                                    NUM_ROW_BLOCK, NUM_COL_BLOCK, NUM_EXPERTS, 
                                    kNumTMAMulticast, kIsTMAMulticastOnA, 16>(row_indices, column_indices, height_offset);
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_index, n_index, num_cols)) {
            // if(threadIdx.x == kNumMathThreads)
            // printf("m_index:%d, n_index:%d, num_rows:%d\n", m_index, n_index, num_rows);
            ...
            for (uint32_t k_iter = 0; k_iter < num_iterations; ++k_iter){
                ...
                if (threadIdx.x == kNumMathThreads){ 
                    ...
                    auto token = cuda::device::barrier_arrive_tx(full_barrier, 1, BLOCK_M * (BLOCK_K / 2) * sizeof(__nv_bfloat16));
                }
                else{
        ...
        while (scheduler.get_next_block(m_index, n_index, num_cols)) {
            ...
            const int *I_panel = reinterpret_cast<const int *>(indices + m_block_idx * NNZ_BLOCK_K);

            // 预取indices
            int num_block = NNZ_BLOCK_K;
            int num_meta_tile = CEIL(num_block, PREFETCHEDCOL); // 每行需要取多少次
            int fetch_meta = 0;
            const int prefetch_meta_stage = PREFETCHEDCOL / BLOCK_PER_TILE; // 取一次可以循环BLOCK_K多少次
            int num_thread_per_blockw = BLOCK_W / LOAD_ELEMENT; // 负责一个block w 长读取的线程数量

            // load the first tile of indice
            ldg_copy<kNumMathThreads>(reinterpret_cast<uint *>(smem_indices), reinterpret_cast<const uint *>(I_panel),
                          min(num_block, PREFETCHEDCOL));
            cutlass::arch::NamedBarrier(kNumMathThreads).sync();
            fetch_meta++;

            #pragma unroll
            for(size_t istage = 0; istage < kNumStages - 1; istage++){

                #pragma unroll
                for(size_t step_load_b = 0; step_load_b < num_iter_b_cp_async; step_load_b++){
                    uint32_t n_offsets = warp_idx * LOAD_WARP_N + (lane_idx / LOAD_GROUP) + step_load_b * (32 / LOAD_GROUP);
                    uint32_t gather_idx_offset = gather_index_ptr[n_offsets];

                    // 恢复全局列坐标
                    int idx = (lane_idx % LOAD_GROUP) / num_thread_per_blockw;
                    uint k_base = smem_indices[idx];
                    int b_k_idx_block = (idx / BN) * BM + k_base;
                    int b_k_idx = b_k_idx_block * BLOCK_W;

                    __nv_bfloat16 *src = rhs + gather_idx_offset * SHAPE_K + b_k_idx + (lane_idx % num_thread_per_blockw) * LOAD_ELEMENT;
                    __nv_bfloat16 *dst = smem_b[istage] + n_offsets * BLOCK_K + (lane_idx % LOAD_GROUP) * LOAD_ELEMENT;

                    bool real_copy = (n_offsets < num_cols);
                    cp_async_srclimit<__nv_bfloat16, 16>(dst, src, real_copy);
                }

                cp_async_group_commit();
                itile2write++;
                itile2write_innstage++;
            }
            ...
            for (uint32_t k_iter = 0; k_iter < num_iterations; ++ k_iter){
                ...
                *((uint2 *)meta_regs) = *((uint2 *)(M_panel + k_iter * (BLOCK_K / 2 / META_ELEMENT) * (BLOCK_M / META_MROW)));
                ...
                // 预取下阶段的列坐标
                if (k_iter % prefetch_meta_stage == 0) { // 读的是下一个阶段的tile indices
                    if (fetch_meta < num_meta_tile) {
                        ...
                        ldg_copy<kNumMathThreads>(reinterpret_cast<uint *>(shared_tile_I), reinterpret_cast<const uint *>(tile_I), meta_num);
                        cutlass::arch::NamedBarrier(kNumMathThreads).sync();
                    }
                    fetch_meta++;
                }
                ...
                int k_idx = k_iter * (BLOCK_K / 2);
                uint *shared_tile_I = smem_indices + (fetch_meta % 2) * PREFETCHEDCOL;
                uint *tile_I = shared_tile_I + (k_iter % prefetch_meta_stage) * BLOCK_PER_TILE;
                    ...
                    if (compute_idx == 0){
                        if(itile2write < num_iterations){
                            ...
                            for(size_t step_load_b = 0; step_load_b < num_iter_b_cp_async; step_load_b++){
                                ...
                                __nv_bfloat16 *src = rhs + gather_idx_offset * SHAPE_K + b_k_idx + (lane_idx % num_thread_per_blockw) * LOAD_ELEMENT;
                                __nv_bfloat16 *dst = smem_b[itile2write_innstage] + n_offsets * BLOCK_K + (lane_idx % LOAD_GROUP) * LOAD_ELEMENT;
                            ...
                            cp_async_group_commit();
                            itile2write++;
                            itile2write_innstage = (itile2write_innstage + 1) % kNumStages;
                        }
                    }
                ...
                cp_async_wait_group<kNumStages - 2>();//允许有NUM_STAGE - 2个commit在运行 发射NUM_STAGE - 1, 保留-2 即保证第一个完成
                itile2read_innstage = (itile2read_innstage + 1) % kNumStages;
            ...
            for (uint32_t store_idx = 0; store_idx < iter_store; ++ store_idx) {
                ...
                uint8_t* smem_ptr = reinterpret_cast<uint8_t*>(
                    smem_d + warp_idx * MMA_M * BLOCK_N + lane_idx * BLOCK_N + (store_idx * MMA_N + lane_idx % 8 * 8) % BLOCK_N
                );

                // NOTES: only 16 lanes' addresses are used
                SM90_U32x2_STSM_N<half2>::copy(
                    __float22half2_rn({shifted_accum[0], shifted_accum[1]}),
                    __float22half2_rn({shifted_accum[2], shifted_accum[3]}),
                    smem_ptr
                );
            ...
            auto block_offsets = (scheduler.current_iter) * gridDim.x + blockIdx.x;

            constexpr int THREADS_PER_ROW = 16;
            constexpr int BF16_PER_THREAD = 8;

            constexpr int WARPS_PER_BLOCK = kNumMathThreads / 32;
            constexpr int ROWS_PER_WARP   = 2;
            constexpr int ROWS_PER_ITER   = WARPS_PER_BLOCK * ROWS_PER_WARP;

            int row_in_warp = lane_idx / THREADS_PER_ROW;
            int lane_in_row = lane_idx % THREADS_PER_ROW;

            #pragma unroll
            for (int iter = 0; iter < 128; iter += ROWS_PER_ITER) {

                int row_id = iter + warp_idx * ROWS_PER_WARP + row_in_warp;

                if (row_id < num_cols) {

                    __nv_bfloat16* gptr = out
                        + n_index * NUM_ROW_BLOCK * BLOCK_M
                        + (block_offsets % NUM_ROW_BLOCK) * BLOCK_N
                        + row_id * BLOCK_N;

                    __nv_bfloat16* sptr = smem_d
                        + row_id * BLOCK_N;

                    int elem_offset = lane_in_row * BF16_PER_THREAD;

                    reinterpret_cast<float4*>(gptr + elem_offset)[0] =
                        reinterpret_cast<float4*>(sptr + elem_offset)[0];
                }
            }
            ...
            //         out + n_index * NUM_ROW_BLOCK * BLOCK_M + (block_offsets % NUM_ROW_BLOCK) * num_cols * BLOCK_M + threadIdx.x * BLOCK_M,
