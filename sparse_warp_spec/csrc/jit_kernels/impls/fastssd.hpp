...
class SSDRuntime final: public LaunchRuntime<SSDRuntime> {
public:
  struct Args {
  ...
  static std::string generate_impl(const Args &args) {
    ...
    #include <sparse_gemm/impls/fastssd.cuh>
    
    using namespace sparse_gemm;
    
    static void __instantiate_kernel() {{
        auto ptr = reinterpret_cast<void*>(&fastssd_impl<
            {}, {}, {},
            {}, {},
            {},
            {},
  ...
  static void launch_impl(const cudaKernel_t &kernel,
    ...
    DG_CUDA_RUNTIME_CHECK(cudaLaunchKernelEx(&config, kernel, 
        ...
        args.meta, args.indices, args.values, args.rhs, args.out, args.a_desc
      ));
  }
};
...
static void fastssd(
    ...
    const auto &code = SSDRuntime::generate(args);
    const auto &runtime = compiler->build("fastssd", code);
    SSDRuntime::launch(runtime, args);
}

// breakdown kernel

class SSDNaiveRuntime final: public LaunchRuntime<SSDNaiveRuntime> {
public:
  struct Args {
    int m, n, k, num_groups;
    // BCSR
  ...
  static std::string generate_impl(const Args &args) {
    ...
    #include <sparse_gemm/impls/fastssd_naive.cuh>
...
static void fastssd_naive(
    ...
    const auto &a_desc = make_sparse_a_desc(values, m, nnz_k,
                                          config.block_m, config.block_k / 2,
                                          config.smem_config.swizzle_a_mode);
    const auto &b_desc = make_1d_desc(rhs, n, k,
                                        config.block_n, config.block_k,
                                        config.smem_config.swizzle_a_mode);
    // Launch
    const SSDNaiveRuntime::Args args = {
        .m = m,
        .n = n,
        .k = aligned_k,
        .num_groups = 1,
    ...
    const auto &code = SSDNaiveRuntime::generate(args);
    const auto &runtime = compiler->build("fastssd", code);
    SSDNaiveRuntime::launch(runtime, args);
}
