...
static CUtensorMapSwizzle mode_into_tensor_map_swizzle_bfloat32(const int& mode) {
    auto size = mode * 2;
    switch (size) {
        case   0: return CU_TENSOR_MAP_SWIZZLE_NONE;
        case  16: return CU_TENSOR_MAP_SWIZZLE_NONE;
...
static CUtensorMap make_1d_desc(const torch::Tensor& t,
                                     const int& shape_m, const int& shape_k,
                                     const int& block_m, const int& block_k,
                                     const int& swizzle_mode) {
    // const auto& elem_size = static_cast<int>(t.element_size());
    ...
    // const cuuint64_t gmem_dims[2] = {static_cast<cuuint64_t>(shape_m), static_cast<cuuint64_t>(shape_k)};
    // const cuuint32_t smem_dims[2] = {static_cast<cuuint32_t>(block_m), static_cast<cuuint32_t>(block_k)};
    // const cuuint64_t gmem_strides[1] = {static_cast<cuuint64_t>(shape_k * elem_size), };
    // const cuuint32_t elem_strides[2] = {1, 1};
    // if (get_env<int>("DG_JIT_DEBUG")) {
    //     printf("Making TMA desc: global memory: %d %d, shared memory: %d %d, outer stride: %d, swizzle: %d, elem size: %d\n",
    //            shape_m, shape_k, 1, block_k,
    //            shape_k, swizzle_mode, elem_size);
    // }
    // cuTensorMapEncodeTiled(
    //     &tensor_map, aten_dtype_to_tensor_map_dtype(t.scalar_type()),
    //     rank, t.data_ptr(), gmem_dims, gmem_strides, smem_dims, elem_strides,
    //     CU_TENSOR_MAP_INTERLEAVE_NONE, mode_into_tensor_map_swizzle(swizzle_mode),
    //     CU_TENSOR_MAP_L2_PROMOTION_L2_256B, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);

    CUtensorMap tensor_map{};
    constexpr uint32_t rank = 1;
    uint64_t gmem_dims[rank] = {static_cast<uint64_t>(shape_m) * static_cast<uint64_t>(shape_k)};
    uint32_t smem_dims[rank] = {static_cast<uint32_t>(block_k)};
    ...
    if (res != CUDA_SUCCESS) {
        ...
        printf("Error initializing tensor map: %s\n", err_str);
    }
    return tensor_map;
}

static CUtensorMap make_sddmm_b_desc(const torch::Tensor& t,
                                     const int& shape_k, const int& shape_n,
                                     const int& block_k, const int& block_n,
                                     const int& swizzle_mode) {
    // const auto& elem_size = static_cast<int>(t.element_size());

    // CUtensorMap tensor_map;
    // constexpr uint32_t rank = 2;
    // const cuuint64_t gmem_dims[2] = {static_cast<cuuint64_t>(shape_k), static_cast<cuuint64_t>(shape_n)};
    // const cuuint32_t smem_dims[2] = {static_cast<cuuint32_t>(block_k), static_cast<cuuint32_t>(1)};
    // const cuuint64_t gmem_strides[1] = {static_cast<cuuint64_t>(1 * elem_size), };
    // const cuuint32_t elem_strides[2] = {1, 1};
    // if (get_env<int>("DG_JIT_DEBUG")) {
    //     printf("Making TMA desc: global memory: %d %d, shared memory: %d %d, outer stride: %d, swizzle: %d, elem size: %d\n",
    //            shape_k, shape_n, block_k, block_n,
    //            shape_n, swizzle_mode, elem_size);
    // }
    // (cuTensorMapEncodeTiled(
    //     &tensor_map, aten_dtype_to_tensor_map_dtype(t.scalar_type()),
    //     rank, t.data_ptr(), gmem_dims, gmem_strides, smem_dims, elem_strides,
    ...
    CUtensorMap tensor_map{};
    constexpr uint32_t rank = 2;
    uint64_t gmem_dims[rank] = {static_cast<uint64_t>(shape_k), static_cast<uint64_t>(shape_n)};
    uint32_t smem_dims[rank] = {static_cast<uint32_t>(block_k), static_cast<uint32_t>(block_n)};
    uint64_t stride[rank - 1] = {static_cast<uint64_t>(shape_k) * 2};
...
static CUtensorMap make_sddmm_c_desc(const torch::Tensor& t,
                                     const int& shape_m, const int& shape_n, const int& n_blocks,
                                     const int& block_m, const int& block_n,
                                     const int& swizzle_mode) {
    const auto& elem_size = static_cast<int>(t.element_size());
    ...
    CUtensorMap tensor_map;
    constexpr uint32_t rank = 2;
    const cuuint64_t gmem_dims[2] = {static_cast<cuuint64_t>(shape_m), static_cast<cuuint64_t>(n_blocks * block_n)};
    const cuuint32_t smem_dims[2] = {static_cast<cuuint32_t>(block_m), static_cast<cuuint32_t>(block_n)};
    const cuuint64_t gmem_strides[1] = {static_cast<cuuint64_t>(shape_n * elem_size), };
    const cuuint32_t elem_strides[2] = {1, 1};
    if (get_env<int>("DG_JIT_DEBUG")) {
        printf("Making TMA desc: global memory: %d %d, shared memory: %d %d, outer stride: %d, swizzle: %d, elem size: %d\n",
               shape_m, shape_n, block_m, block_n,
               shape_n, swizzle_mode, elem_size);
    }
    (cuTensorMapEncodeTiled(
        &tensor_map, aten_dtype_to_tensor_map_dtype(t.scalar_type()),
...
static CUtensorMap make_sparse_a_desc(const torch::Tensor &t,
                                        const int &shape_m, const int &shape_k,
                                        const int &block_m, const int &block_k,
                                        const int &swizzle_mode)
{
    // tma的desc必须是row major
    const auto &elem_size = static_cast<int>(t.element_size());
    CUtensorMap tensor_map{};
    // printf("A desc: %d %d %d %d\n", shape_k, shape_m, block_k, block_m);
    // printf("Element size: %d\n", elem_size);
    constexpr uint32_t rank = 2;
    uint64_t gmem_dims[rank] = {static_cast<uint64_t>(shape_k), static_cast<uint64_t>(shape_m)};
    uint32_t smem_dims[rank] = {static_cast<uint32_t>(block_k), static_cast<uint32_t>(block_m)};
    uint64_t stride[rank - 1] = {static_cast<cuuint64_t>(shape_k * elem_size)};
    uint32_t elem_stride[rank] = {1, 1};
    ...
    CUresult res = cuTensorMapEncodeTiled(
        ...
        elem_stride,
        CUtensorMapInterleave::CU_TENSOR_MAP_INTERLEAVE_NONE,
        mode_into_tensor_map_swizzle_bfloat32(swizzle_mode),
        CUtensorMapL2promotion::CU_TENSOR_MAP_L2_PROMOTION_NONE,
        CUtensorMapFloatOOBfill::CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
    if (res != CUDA_SUCCESS)
    {
        const char *err_str = nullptr;
        cuGetErrorString(res, &err_str);
        printf("A Tensor Map Error initializing tensor map: %s\n", err_str);
    }
    return tensor_map;
}


static CUtensorMap make_block_sparse_b_desc(const torch::Tensor& t,
                                     const int& shape_k, const int& shape_n,
                                     const int& block_k, const int& block_n,
                                     const int& swizzle_mode) {
