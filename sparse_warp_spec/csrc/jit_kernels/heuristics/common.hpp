...
template <typename ArchSpec, int BM, int BN, int BK, int Nstages, int PrefetchedCol=512>
static SparseConfig get_sparse_config(
    const GemmType& gemm_type, const KernelType& kernel_type,
    const int& m, const int& n, const int& k, const int& num_groups,
    const int& num_sms
){
    const int block_m = BM, block_k = BK, block_n = BN;
    const int num_stages = Nstages;
    const int &swizzle_a_mode = block_k / 2;
    const int &swizzle_b_mode = block_n;
    const int &swizzle_cd_mode = block_n;

    const int &smem_a_per_stage = block_m * (block_k / 2) * sizeof(__nv_bfloat16); // 使用sparse tensor core,这使得B的尺度是A的两倍
    const int &smem_b_per_stage = block_n * block_k * sizeof(__nv_bfloat16);
    const int &smem_c_per_stage = block_n * block_m * sizeof(__nv_bfloat16);
    const int &smem_barrier = num_stages * 8; // barrier大小为8bytes
    const int &smem_indices = PrefetchedCol * sizeof(int);

    const int smem_size = (smem_a_per_stage + smem_b_per_stage) * num_stages + smem_c_per_stage + smem_barrier + smem_indices * 2;
    
    MulticastConfig multicast_config = {1, true};
    SharedMemoryConfig shared_config = SharedMemoryConfig {
            .smem_size = smem_size,
    ...
    const auto& config = SparseConfig {
        .gemm_type = gemm_type,
