...
class Compiler {
    ...
    std::string get_library_version() const {
        std::stringstream ss;
        for (const auto& f: collect_files(library_include_path / "sparse_gemm")) {
...
public:
    ...
    explicit Compiler(const std::filesystem::path& library_root_path) {
        ...
        // Cache settings
        cache_dir_path = std::filesystem::path(get_env<std::string>("HOME")) / ".sparse_gemm";
...
public:
    NVCCCompiler(const std::filesystem::path& library_root_path,
        ...
        flags = fmt::format("{} -I{} --gpu-architecture=sm_{}a -Xptxas=-v "
