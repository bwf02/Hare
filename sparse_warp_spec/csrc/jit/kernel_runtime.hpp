...
public:
    ...
    explicit KernelRuntime(const std::filesystem::path& dir_path) {
        ...
        const std::vector<std::string> illegal_names = {"vprintf", "__instantiate_kernel", "__internal", "__assertfail", "__cuda_syscall_cp_async_bulk_unicast", "__cuda_syscall_cp_async_bulk_tensor_1d_tile_unicast", "__cuda_syscall_cp_async_bulk_tensor_2d_tile_unicast"};
...
public:
    ...
    static void launch(const std::shared_ptr<KernelRuntime>& kernel_runtime, const Args& args) {
        ...
        int device = 0, max_smem_optin = 0;
        DG_CUDA_RUNTIME_CHECK(cudaDeviceGetAttribute(&max_smem_optin,
                                                    cudaDevAttrMaxSharedMemoryPerBlockOptin,
                                                    device));

        DG_HOST_ASSERT(launch_args.smem_size <= max_smem_optin and "Shared memory size exceeds the maximum limit");
        // Set dynamic shared memory size
        if (launch_args.smem_size > 0 and launch_args.smem_size <= max_smem_optin){
            // std::cout << "Setting dynamic shared memory size: " << launch_args.smem_size << " bytes\n";
