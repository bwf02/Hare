...
class DeepSeekHareMoE(nn.Module):

    def __init__(self, hidden_dim: int, ffn_dim: int, num_experts: int,
                 top_k: int, n_shared_experts=None, bw=16):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with nvtx.annotate("padded_scatter"):
            x = ops.scatter(x, indices, bin_ids, weights, bins, self.top_k)

        with nvtx.annotate("shared_experts"):
            if self.n_shared_experts is not None:
