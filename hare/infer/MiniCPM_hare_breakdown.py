...
from model.mixtral_breakdown import HareMoE
...
parser.add_argument('--repo', type=str, default="openbmb/MiniCPM-MoE-8x2B")
...
def MiniCPM_mlp_run():
    ...
    if args.time:
        ...
        print(
            f"MiniCPM,mlp,hare,{ITER},{args.batch_size},{args.seq_len},"
...
def MiniCPM_decoder_layer_run():
    ...
    if args.time:
        ...
        print(
            f"MiniCPM,layer,hare,{ITER},{args.batch_size},{args.seq_len},"
...
def MiniCPM_model_run():
    ...
    model = MiniCPMModel(configuration).to(device=device, dtype=torch.bfloat16)
    
    decoder_layers = nn.ModuleList()
    
    for i in range(configuration.num_hidden_layers):
        layer = MiniCPMDecoderLayer(configuration, i).to(
            device=device, 
    ...
    if args.time:
        ...
        print(
            f"MiniCPM,model,hare,{ITER},{args.batch_size},{args.seq_len},"
